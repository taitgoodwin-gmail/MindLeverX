...
- Local saved results and eight-page PDF are implemented. Latest MIN-14 follow-up: fixed inconsistent UTF-8 BOM decoding; 75 application tests and public-build checks passed. Original PDF visual/download evidence is in the product checkout's `docs/pdf-results-slice.md`; this process pass does not repeat or substitute for it.
...
- Separate readiness reviewers confirmed the 97 rows remain a review draft and the paid service is not end-to-end. After the coverage draft, a useful proposed engineering bridge is saved evidence → immutable report revision → an actual review-queue entry. Existing local review decisions are not linked to the current saved-results PDF. Prepare the bounded chunk under MIN-7; payment/private delivery and full acceptance remain separate. The MIN-12 clarification allows unknown optional model context without weakening the collection gate.
