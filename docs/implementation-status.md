...
The active product checkout is `/Users/tag/Documents/ChatGPT/MindLeverX evidence-inspection`. In addition to the local saved-results frontend and eight-page PDF, MIN-7 now connects a configured internal subject to immutable source/result/PDF revisions and the actual review queue. Retained downloads survive source changes and restart; local decisions are bound to the displayed revision. See [scope, full requirement narratives, tests and owner walkthrough](report-review-slice.md).

Verification: 114 application tests plus site/platform/public-build checks passed; after an AI reviewer found a mutable evidence-reference instruction in the PDF, the wording and renderer version were corrected and 26 affected tests plus the real-source integration were rerun successfully. Browser preparation, retained answers/PDF, keyboard/narrow layout and synthetic error/retry/decision checks passed. The actual 0/16 saved-sample review remains pending for the owner at port 4329. Latest commit and verified GitHub backup refs are on MIN-7; a code backup does not include the private source or temporary SQLite DB and is not deployment or customer acceptance.
